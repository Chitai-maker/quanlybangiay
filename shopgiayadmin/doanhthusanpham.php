...
SELECT 
    g.tengiay,
    SUM(
        (ct.soluong / tong.tong_soluong) * dh.tongtien
    ) AS doanhthu
FROM donhang dh
JOIN chitietdonhang ct ON dh.ma_donhang = ct.ma_donhang
JOIN giay g ON ct.ma_giay = g.magiay
JOIN (
    SELECT ma_donhang, SUM(soluong) AS tong_soluong
    FROM chitietdonhang
    GROUP BY ma_donhang
) tong ON tong.ma_donhang = dh.ma_donhang
$where_sql
GROUP BY g.tengiay
...
<div class="container mt-4">
    <h2 class="mb-4 text-center"><i class="fa fa-cube"></i> Báo Cáo Doanh Thu Theo Mặt Hàng</h2>
    ...
    <?php if ($max): ?>
        <div class="alert alert-success">
            <b>Cao nhất:</b> <?= htmlspecialchars($max['tengiay']) ?> – <?= number_format($max['doanhthu'], 0, ',', '.') ?> VND
        </div>
    <?php endif; ?>
    <?php if ($min && $min['doanhthu'] > 0): ?>
        <div class="alert alert-warning">
            <b>Thấp nhất:</b> <?= htmlspecialchars($min['tengiay']) ?> – <?= number_format($min['doanhthu'], 0, ',', '.') ?> VND
        </div>
    <?php endif; ?>
